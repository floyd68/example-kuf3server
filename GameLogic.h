...
public:
	...
	{
		// by floyd
		// GameLogic이라는 클래스에서 game_logic_mgr를 생성하고 소멸시킨다는 얘기는
		// gmae_logic_mgr은 GameLogic이 관리한다는 얘기고 그건 곧 game_logic_mgr은 GameLogic에 속한 클래스라는 얘기임
		...
		// 1-1. GameLogic에서 get_game_logic_mgr() 같은 함수는 절대 절대 만들지 말아야 함. 이 함수를 만들어야 할 정도로 외부에서 접근이 많다면 2번 방법을 고려해야함
		// 2. game_logic_mgr을 외부에서 접근할 일이 많다면 GameLogic내에서 생성하는게 아니라 Main쪽에서 생성해야함. 
